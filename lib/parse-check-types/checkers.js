  ...
  complex: {
    array: (parsedType, value, key, ast, filters, validator, check) => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        const result = check(types, value[i], i, ast, {}, validator, false)
    ...
    object: (parsedType, value, key, ast, filters, validator, check) => {
      ...
      for (const pair of pairs) {
        for (const valueKey in value) {
          let result = check(
            pair.keyTypes, valueKey, valueKey, ast, filters, validator, false
          )
    ...
    typedef: (
      parsedType, value, key, ast, filters, validator, check, throwOnError
    ) => {
          ...
          if (
            ...
            if (typedef.elements.callback) {
              ...
              checkResult = validator.validateParams(
                propertyTagName, typedef, value, throwOnError
              )
            }
