  ...
  complex: {
    ...
    typedef: (
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (i === 0) typedefs = ast.localTypedefs
        else if (i === 1) typedefs = ast.importedTypedefs
        else if (i === 2) typedefs = ast.ambientTypedefs

        for (const typedef of typedefs) {
          const element = typedef.elements.typedef ?? typedef.elements.callback

          if (element?.name !== parsedType.typeExpression) {
            continue
          }

          /** @type {'property' | 'param'} */
          const propertyTagName = typedef.elements.callback
            ? 'param'
            : 'property'
          ...
          if (
            ...
            if (typedef.elements.callback) {
              checkResult = validator.validateFunctionArguments(
                typedef, value, forceStrict
              )
            }
            else {
              checkResult = (throwOnError)
                ? validator.validate(typedef, value, forceStrict)
                : validator.check(typedef, value, forceStrict)
            }
