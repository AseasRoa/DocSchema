...
const checkers = {
  ...
  complex: {
    array : (parsedType, value, key, typedefs, filters, check) => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        const result = check(types, value[i], i, typedefs, {})
    ...
    object : (parsedType, value, key, typedefs, filters, check) => {
      ...
      for (const pair of pairs) {
        for (const valueKey in value) {
          let result = check(pair.keyTypes, valueKey, valueKey, typedefs, filters)

          if (!result) return false

          result = check(pair.valueTypes, value[valueKey], valueKey, typedefs, filters)
    ...
    objectLiteral : (parsedType, value, key, typedefs, filters, check) => {
      if (!(value instanceof Object)) {
        return false
      }
    ...
    typedef : (parsedType, value, key, typedefs, filters, check) => {
      // 1) Try typedefs
