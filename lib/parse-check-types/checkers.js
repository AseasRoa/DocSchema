...
const checkers = {
  ...
  complex: {
    array : (parsedType, value, key, typedefs, filters, validator, check) => {
      ...
      for (let i = 0; i < value.length; i++) {
        ...
        const result = check(types, value[i], i, typedefs, {}, validator)
    ...
    object : (parsedType, value, key, typedefs, filters, validator, check) => {
      ...
      for (const pair of pairs) {
        for (const valueKey in value) {
          let result = check(
            pair.keyTypes, valueKey, valueKey, typedefs, filters, validator
          )

          if (!result) return false

          result = check(
            pair.valueTypes, value[valueKey], valueKey, typedefs, filters, validator
          )
    ...
    objectLiteral : (parsedType, value, key, typedefs, filters, validator, check) => {
      if (!(value instanceof Object)) {
        return false
      }
    ...
    typedef : (parsedType, value, key, typedefs, filters, validator, check) => {
      // 1) Try typedefs
      ...
      for (const typedef of typedefs) {
        ...
        const propertyTags = typedef.elements.property
        // const typeName = typedef.elements.typedef?.types[0]?.typeName

        /**
         * Variant 1: Use the @property or @param tags of the typedef
         */
        ...
        if (
          ...
          /*
           * && (
           *   // no type definition, for example: @typedef TypeName
           *   !typeName
           *   // has type definition, but it's object, for example: @typedef {Object} TypeName
           *   || typeName === 'object'
           *   || typeName === 'any'
           * )
           */
          ...
          const checkResult = validator.validateTypedef(typedef, value, false)
          
          return checkResult.pass
        }

        const parsedTypes = typedef.elements.typedef?.types ?? []
