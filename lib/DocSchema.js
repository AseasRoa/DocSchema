...
class DocSchema {
   ...
   */
  validate(value) {
    this.#validateOrCheck(value, true)
  ...
  #validateOrCheck(value, throwOnError) {
    ...
    if (elements.enum) {
      return docSchemaValidator.validateTag(
        'enum', this.#ast, value, throwOnError, this.#ast.strict
      )
    }

    if (elements.param.length > 0) {
      return docSchemaValidator.validateParams(
        'param', this.#ast, value, throwOnError, this.#ast.strict
      )
    }

    if (elements.typedef) {
      return docSchemaValidator.validateTypedef(
        this.#ast, value, throwOnError, this.#ast.strict
      )
    }

    if (elements.type) {
      return docSchemaValidator.validateTag(
        'type', this.#ast, value, throwOnError, this.#ast.strict
      )
    }
