...
const docSchemaParser = new DocSchemaParser(filesReader)
...
export class DocSchema {
  ...
  #create() {
    ...
    const { contents, linesInfo } = filesReader.readFile(this.#fileName)

    let index = 0
    // Where ( of docSchema() or new DocSchema() is in the code
    let openingParenthesesIndex = 0
    // Where ) of docSchema() or new DocSchema() is in the code
    let closingParenthesesIndex = 0

    if (linesInfo) {
      /**
       * In case of:
       * - docSchema() - index of (
       * - new DocSchema() - index of n
       */
      index = (linesInfo[this.#line] ?? 0) + (this.#column - 1)

      const openingBrace = '('
      openingParenthesesIndex = contents.indexOf(openingBrace, index)
      const contentsFromOpeningBrace = contents.slice(openingParenthesesIndex)

      if (contentsFromOpeningBrace[0] !== openingBrace) {
        throw new Error('The first character of this contents slice must be "("')
      }

      closingParenthesesIndex = openingParenthesesIndex
        + findClosingBracketPosition(contentsFromOpeningBrace)
    }
    ...
    for (const comment of comments) {
      if (comment.startIndex > closingParenthesesIndex) {
        break
      }

      if (
        (!comment.elements.type && this.#line === comment.endLine + 1)
        || (
          comment.elements.type
          && comment.startIndex > openingParenthesesIndex
          && comment.endIndex < closingParenthesesIndex
        )
      ) {
        ast = comment
