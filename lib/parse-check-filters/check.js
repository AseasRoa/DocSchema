...
const checkersWrappers = {
  array: (filters, value) => {
    for (const key in filters) {
      // Not necessary, because these problems should be caught at parse time
...
function check(filters, value) {
  ...
  lastError.kind = 'filter'
  lastError.message = `Expected "${value}" to be any of these types ${expectedTypes}`
