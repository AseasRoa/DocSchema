...
describe('Validate Filters', () => {
  ...
  describe('string', () => {
    ...
    test('url', () => {
      // url
      expect(check({url: true}, 'https://github.com')).toBe(true)
      expect(check({url: false}, '1234')).toBe(true)

      // url wrong
      expect(check({url: true}, '1234')).toBe(false)
      expect(check({url: false}, 'https://github.com')).toBe(false)
    })
