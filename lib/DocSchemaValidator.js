...
class DocSchemaValidator {
  ...
  validateFunctionArguments(ast, args, throwOnError = true) {
    /*
     * Maybe @type pointing to a @callback is used, like this: @type {MyCallback}
     */
    ...
    for (const parsedTag of parsedTags) {
      ...
      const result = this.#validate(
        'argument', parsedTag, arg, id, ast.typedefs, throwOnError
      )
  ...
  validateParams(name, ast, value, throwOnError = true) {
    ...
    for (const parsedTag of parsedTags) {
      ...
      const result = this.#validate(
        name, parsedTag, val, key, ast.typedefs, throwOnError
      )
