...
class DocSchemaValidator {
  ...
  validateFunctionArguments(
    ...
    if (ast.elements.type && ast.elements.param.length === 0) {
      return this.validateTag('type', ast, args, throwOnError, forceStrict)
    }
   ...
   */
  validateParams(name, ast, value, throwOnError = true, forceStrict = false) {
    startCheckResult()
   ...
   * Throws Error if the input AST is not valid.
   */
  validateTag(tagName, ast, value, throwOnError = true, forceStrict = false) {
    startCheckResult()
   ...
   * Throws Error if the input AST is not valid.
   */
  validateTypedef(ast, value, throwOnError = true, forceStrict = false) {
    startCheckResult()
   ...
   * @throws {ValidationError}
   */
  #validate(tagName, parsedTag, value, key, ast, throwOnError, forceStrict) {
    const result = check(
      parsedTag.types,
      value,
      key,
