...
class DocSchemaValidator {
  ...
  validateFunctionArguments(ast, args, throwOnError = true) {
    ...
    for (const param of params) {
      ...
      const result = typesValidator(types, arg, ast.typedefs, filters)
  ...
  validateParams(name, ast, value, throwOnError = true) {
    ...
    for (const param of params) {
      ...
      const result = typesValidator(param.types, val, ast.typedefs, param.filters)
  ...
  validateTag(tagName, ast, value, throwOnError = true) {
    ...
    const result = typesValidator(types, value, ast.typedefs, filters)
 ...
 */
function typesValidator(parsedTypes, value, typedefs, filters) {
  for (const parsedType of parsedTypes) {
    // @ts-ignore
    const validatorFunc =
      validators.simple[parsedType.typeName] ?? validators.complex[parsedType.typeName]

    if (!(validatorFunc instanceof Function)) {
      throw new Error(`Wrong typeName ${parsedType.typeName}`)
    }

    const result = validatorFunc(parsedType, value, typedefs, filters, typesValidator)

    if (result) {
      // Filters validation
      if (parsedType.typeName in filtersValidators) {
        const filtersValidatorFunc = filtersValidators[parsedType.typeName]

        filtersValidatorFunc(filters, value)
      }
