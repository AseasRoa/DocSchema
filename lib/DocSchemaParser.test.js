...
describe('DocSchemaParser', () => {
  describe('Comments Discovery', () => {
    ...
    test('detect typedefs', () => {
      ...
      const typedefs = ast?.localTypedefs

      expect(typedefs[0]).toMatchObject({
        startLine: 2,
        endLine: 4,
        elements: { typedef: { description: 'StringTypedef' }, strict: false }
      })

      expect(typedefs[1]).toMatchObject({
        startLine: 6,
        endLine: 8,
        elements: { typedef: { description: 'ObjTypedefOne' }, strict: false }
      })

      expect(typedefs[2]).toMatchObject({
        startLine: 10,
        endLine: 16,
        elements: { typedef: { description: 'ObjTypedefTwo' }, strict: true }
      })
    })
