...
const validators = {
  ...
  complex: {
    array : (parsedType, value, typedefs, filters, validate) => {
    ...
    object : (parsedType, value, typedefs, filters, validate) => {
      ...
      for (const pair of pairs) {
        for (const key in value) {
          let result = validate(pair.keyTypes, key, typedefs, filters)

          if (!result) return false

          result = validate(pair.valueTypes, value[key], typedefs, filters)
    ...
    objectLiteral : (parsedType, value, typedefs, filters, validate) => {
      if (!(value instanceof Object)) {
        return false
      }
    ...
    typedef : (parsedType, value, typedefs, filters, validate) => {
      // 1) Try typedefs
