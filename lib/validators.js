...
const validators = {
  ...
  complex: {
    array : (parsedType, value, typedefs, filters, typesValidator) => {
    ...
    object : (parsedType, value, typedefs, filters, typesValidator) => {
      ...
      for (const pair of pairs) {
        for (const key in value) {
          let result = typesValidator(pair.keyTypes, key, typedefs, filters)

          if (!result) return false

          result = typesValidator(pair.valueTypes, value[key], typedefs, filters)
    ...
    objectLiteral : (parsedType, value, typedefs, filters, typesValidator) => {
      if (!(value instanceof Object)) {
        return false
      }
    ...
    typedef : (parsedType, value, typedefs, filters, typesValidator) => {
      // 1) Try typedefs
