...
class DocSchemaParser {
        ...
        for (let importedFile of importedFiles) {
          ...
          for (const ast of parsedCommentsFromImport) {
            for (const typedef of ast.typedefs) {
              for (const parsedComment of parsedComments) {
                if (parsedComment.typedefs.includes(typedef)) {
                  continue
                }

                parsedComment.typedefs.push(typedef)

                break // typedef objects are the same object in all comments
              }
            }
  ...
  #extractCommentsInfo(code) {
    ...
    const output = []
    /**
     * It's important to consider the \r characters in the pattern below
     *
     * @type {RegExp}
     */
    const pattern = /(\/\*\*[\t ]*\r?\n(?:[ \t]* \*.*\r?\n)+[ \t*]*\*\/|\/\*\*.*\*\/)(?:\s*\r?\n)*(\s*[^\s\/].*)?/ug
  ...
  #extractTagComponents(
    ...
    parsedTag.types = parseTypes(parsedTag.typeExpression, this.#currentLocation)

    // 5) Search for description and filters
    if (hasDescription && withoutTagName) {
      const { description, filters } = parseFilters(
        withoutTagName, parsedTag.types, this.#currentLocation
      )
  ...
  #parseSingleLineTag(choppedComment, tagName) {
    ...
    for (const line of tagLines) {
      ...
      /*
       * // I disabled this, because it didn't work well with typedef row without type
