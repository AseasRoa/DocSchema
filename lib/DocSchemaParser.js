...
import { separateDescriptionAndFilters } from './filters.js'
...
class DocSchemaParser {
  ...
  #extractTagComponents(
    ...
    if (hasDescription && withoutTagName) {
      const result = separateDescriptionAndFilters(withoutTagName, parsedTag.types)

      parsedTag.description = result.description
      parsedTag.filters = result.filters
    }
