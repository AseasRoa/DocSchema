...
import { FilesReader } from './FilesReader.js'
import {
...
export class DocSchemaParser {
  ...
  /**
   * @param {FilesReader} [filesReader]
   * Files reader. It's optional, so when the parser is used from outside, it
   * would use its own files reader
   */
  constructor(filesReader) {
    this.#filesReader = filesReader ?? new FilesReader()
  }
  ...
  parseComments(code, file = '') {
    const commentsInfo = this.#extractCommentsInfo(code, file)
  ...
  parseFile(file) {
    ...
    // To prevent recursive file imports, this must be created here
    // and immediately added to the cache.
    // This ensures that it is filled with data only once.
    const parsedComments = []
    ...
    if (!astCacheForFiles.has(file)) {
      ...
      let dir = ''
      const { contents } = this.#filesReader.readFile(file)
      const parsedCommentsLocal = this.parseComments(contents, file)

      parsedComments.push(...parsedCommentsLocal)
      ...
      if (importedFiles.length > 0) {
        ...
        for (let importedFile of importedFiles) {
          ...
          const parsedCommentsFromImport
            = astCacheForFiles.get(importedFile) ?? this.parseFile(importedFile)
      ...
      for (const parsedComment of parsedComments) {
        ...
        for (const importElement of elementsImport) {
          ...
          for (const ast of parsedCommentsFromImport) {
            const typedefName = ast.elements.typedef?.name

            if (!typedefName) {
              continue
            }

            if (typedefsToInclude.includes(typedefName)) {
      ...
      /**
       * Process import
       * Example: {import('./types.js').MyType}
       */

      for (const parsedComment of parsedComments) {
        if (parsedComment.elements.typedef) {
          for (const type of parsedComment.elements.typedef.types) {
            if (type.typeName !== 'directImport' || !type.directImport) {
              continue
            }

            if (!type.directImport.file.startsWith('.')) {
              throw new Error(
                `Cannot import type ${type.directImport.typeName} from path ${type.directImport.file}
    at ${parsedComment.file}:${parsedComment.startLine}
    Only relative paths are allowed.`
            ...
            const importedFile = joinPath(dir, type.directImport.file)
            ...
            try {
              ...
              for (const ast of parsedCommentsFromImport) {
                ...
                if (typedefName === type.directImport.typeName) {
                  const { directlyImportedTypedefs } = parsedComment

                  if (!(directlyImportedTypedefs.includes(ast))) {
                    parsedComment.directlyImportedTypedefs.push(ast)
                  }
   ...
   */
  #extractCommentsInfo(code, file) {
    const linesInfo = (file)
      ? this.#filesReader.readFile(file).linesInfo
      : this.#filesReader.extractLinesInfo(code)
    ...
    while (true) {
      ...
      const startLine = binarySearchFirstGTE(linesInfo, startIndex)
      const endLine = binarySearchFirstGTE(linesInfo, endIndex)
      const startLineIndex = (linesInfo[startLine] ?? 0)
      const endLineIndex = (linesInfo[endLine] ?? 0)
      const startColumn = startIndex - startLineIndex
      const endColumn = endIndex - endLineIndex
  ...
  #parseChoppedComment(choppedComment) {
    ...
    return {
      ...
      callback: (usedTags.has('callback'))
        ? this.#parseSingleLineTag(choppedComment, 'callback', true)
        : null,
      typedef: (usedTags.has('typedef'))
        ? this.#parseSingleLineTag(choppedComment, 'typedef', true)
        : null,
   ...
   * @throws {SyntaxError | Error}
   */
  #parseSingleLineTag(choppedComment, tagName, hasName = false) {
