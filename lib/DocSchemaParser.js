...
import { parse as parseTypes } from './parse-validate-types/parse.js'
import { parse as parseFilters } from './parse-validate-filters/parse.js'
...
class DocSchemaParser {
  ...
  #extractTagComponents(
    ...
    parsedTag.types = parseTypes(parsedTag.typeExpression)

    // 5) Search for description and filters
    if (hasDescription && withoutTagName) {
      const { description, filters } = parseFilters(withoutTagName, parsedTag.types)
